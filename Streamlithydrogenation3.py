...
selectivity = st.number_input("Enter the selectivity rate (0.1-1)", value=0.95, min_value=0.1, max_value=1.0)  # Selectivity rate (95%)
conversion_rate = st.number_input("Enter the conversion rate (0.1-1)", value=0.9, min_value=0.1, max_value=1.0)  # Conversion rate (90%)

# Option for Recycling Toggle
recycling_enabled = st.radio("Enable Recycling?", options=["Yes", "No"], index=0)
recycling_rate = st.number_input("Enter the recycling rate (0.1-1)", value=0.85, min_value=0.1, max_value=1.0) if recycling_enabled == "Yes" else 0
