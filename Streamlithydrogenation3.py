...
# User Inputs
target_mch_yield = st.number_input("Enter the target cumulative yield of MCH in kg", value=95000)  # Target cumulative yield of MCH in kg
toluene_flow_rate = st.number_input("Enter the increment of toluene added each hour in kg", value=60000)  # Increment of toluene added each hour in kg
temperature = st.number_input("Enter the temperature in °C for the catalyst", value=200)  # Set reaction temperature in °C
pressure = st.number_input("Enter the pressure, barg (assumed constant)", value=10)  # Assume constant pressure for simplicity
max_yield_ratio = 1.0  # Max MCH yield per kg of toluene (theoretical)

# Selectivity, Conversion Rates, and Recycling Rate
selectivity = st.number_input("Enter the selectivity rate (0-1)", value=0.95)  # Selectivity rate (95%)
conversion_rate = st.number_input("Enter the conversion rate (0-1)", value=0.9)  # Conversion rate (90%)

# Option for Recycling Toggle
recycling_enabled = st.radio("Enable Recycling?", options=["Yes", "No"], index=0)
recycling_rate = st.number_input("Enter the recycling rate (0-1)", value=0.85) if recycling_enabled == "Yes" else 0
